...
def on_entry_click(event):
    if url_entry.get() == placeholder_text:
        ...
        url_entry.config(fg="black")  # Change text color to white

def on_focus_out(event):
    if not url_entry.get().strip():
        url_entry.insert(0, placeholder_text)
        url_entry.config(fg="black")  # Change text color to gray

def analyze_url():
    url = url_entry.get().strip()
    if url == placeholder_text or url == "":
...
title_label = tk.Label(root, text="URL Analyzer", 
                       font=("Arial", 20, "bold"),
                       fg="white", bg="#16A085")
